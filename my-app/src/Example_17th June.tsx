...
export const Example = () => {
    /* const conn = ElectricDatabase.init('my.db');
    const config = {
        url: "http://localhost:5133",
        debug: true
      }
const electric = electrify(conn, schema, config); */

/* if (config.debug) {
  const { addToolbar } = import('@electric-sql/debug-toolbar')
  addToolbar(electric)
} */
  ...
  function collectIdentifiers() {
    spreadsheetWithIds = [];
    console.log("here inside collectIdentifiers");
    if (colmap.results && colmap.results.length == 0) {
        ...
        if (rowmap.results.length !=0) {
            ...
            for ( let j = 0; j < rowmap.results.length; j++) {
                spreadsheetWithIds.push(['', '', rowmap.results[j].id, rowmap.results[j].pos, '']);
                console.log("spreadsheetwithIDs", spreadsheetWithIds);
            }
        }
    } else if (colmap.results && colmap.results.length != 0 && rowmap.results.length !=0) {
            ...
            for (let i = 0; i < colmap.results.length; i++) {
                for (let j =0; j < rowmap.results.length; j++) {
                  if (content.results.length !=0 ) {
                    for ( let k = 0; k < content.results.length; k++) {
                      if (colmap.results[i].id == content.results[k]['colindex'] && rowmap.results[j].id == content.results[k]['rowindex']) {
                        if ( content.results[k]['content'] ) {
                          spreadsheetWithIds.push([colmap.results[i].id, colmap.results[i].pos, rowmap.results[j].id, rowmap.results[j].pos, content.results[k]['content']]);
                          console.log("spreadsheetwithIDs", spreadsheetWithIds);
                        } else {
                          spreadsheetWithIds.push([colmap.results[i].id, colmap.results[i].pos, rowmap.results[j].id, rowmap.results[j].pos, '']);
                          //console.log("spreadsheetwithIDs", spreadsheetWithIds);
                      }
                      }
                        //console.log("colmap.results[i].id , content.results[k]['colindex']", colmap.results[i].id, content.results[k]['colindex'], rowmap.results[j].id, content.results[k]['rowindex'])
    ...
    } else if (rowmap.results && rowmap.results.length == 0 && colmap.results.length != 0) {
  ...
  function rebuildSpreadsheet() {
    ...
    let newSpreadsheet = [];
    if (!rowmap.results) {
      newSpreadsheet = Array(ySpreadsheet.length);
    }else if (rowmap.results.length != 0) {
      newSpreadsheet = Array(rowmap.results.length);
    }
    
    for (let i = 0; i < ySpreadsheet.length; i++) {
      if (colmap.results.length == 0 ) {
        newSpreadsheet[i] = Array.apply(null, ...Array(colmap.results.length)).map(function () {return undefined});
      } else {
    ...
    for ( let k = 0; k < spreadsheetWithIds.length; k++ ) {
      for ( let i = 0; i < colmap.results.length; i++ ) {
        for ( let j = 0; j < rowmap.results.length; j++ ) {
          if (i == spreadsheetWithIds[k][1] && j == spreadsheetWithIds[k][3]) {
            if (!newSpreadsheet[j]) {
              console.log(typeof newSpreadsheet[j])
              newSpreadsheet[j] = Array.apply(null, ...Array(colmap.results.length)).map(function () {return ' '});
            }
            
            newSpreadsheet[j][i] = spreadsheetWithIds[k][4];
          }
        }
      }
    }
    if(newSpreadsheet)
      console.log("after rebuild", newSpreadsheet);
